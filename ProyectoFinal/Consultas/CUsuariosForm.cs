...
namespace ProyectoFinal.Consultas
{
    ...
    {
        Expression<Func<Us, bool>> filter = x => true;
        ...
        private void CUsuariosForm_Load(object sender, EventArgs e)
        {

        }

        private void FiltrarButton_Click_1(object sender, EventArgs e)
        {
            switch (CFiltrarComboBox.SelectedIndex)
            {
                case 0:
                    int id = int.Parse(CriterioBusquedaTextBox.Text);
                    filter = (x => x.UsuarioId == id && ((x.Fecha >= DesdeDateTimePicker.Value) && (x.Fecha <= HastaDateTimePicker.Value)));
                    break;
                case 1:
            ...
            ConsultaDataGridView.DataSource = UsuariosBLL.GetList(filter);
        }
    }
}
